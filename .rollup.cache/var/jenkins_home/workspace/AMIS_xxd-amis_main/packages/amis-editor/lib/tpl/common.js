...
import { setSchemaTpl, getSchemaTpl, defaultValue, isObject, tipedLabel } from 'amis-editor-core';
...
setSchemaTpl('formulaControl', function (schema) {
    ...
    return __assign({ type: 'ae-formulaControl', variableMode: 'tree' }, schema);
});
setSchemaTpl('expressionFormulaControl', function (schema) {
    if (schema === void 0) { schema = {}; }
    return __assign({ type: 'ae-expressionFormulaControl', variableMode: 'tree' }, schema);
});
setSchemaTpl('textareaFormulaControl', function (schema) {
    if (schema === void 0) { schema = {}; }
    return __assign({ type: 'ae-textareaFormulaControl', variableMode: 'tree' }, schema);
});
setSchemaTpl('tplFormulaControl', function (schema) {
    if (schema === void 0) { schema = {}; }
    return __assign({ type: 'ae-tplFormulaControl', variableMode: 'tree' }, schema);
});
setSchemaTpl('DataPickerControl', function (schema) {
    if (schema === void 0) { schema = {}; }
    return __assign({ type: 'ae-DataPickerControl' }, schema);
});
...
setSchemaTpl('collapseGroup', function (config) {
    var collapseGroupBody = config
        ...
        .map(function (item) {
        var _a;
        return (__assign(__assign({ type: 'collapse', headingClassName: 'ae-formItemControl-header ae-Collapse-header', bodyClassName: 'ae-formItemControl-body' }, item), { collapsed: (_a = item.collapsed) !== null && _a !== void 0 ? _a : false, key: item.title, body: flatten(item.body) }));
    });
    return {
        type: 'collapse-group',
...
setSchemaTpl('valueFormula', function (config) {
    ...
    return {
        ...
        body: [
            getSchemaTpl('formulaControl', __assign({ label: label !== null && label !== void 0 ? label : '默认值', name: name || 'value', rendererWrapper: rendererWrapper, needDeleteProps: needDeleteProps, valueType: valueType, header: header, placeholder: placeholder, rendererSchema: curRendererSchema, variables: variables, requiredDataPropsVariables: requiredDataPropsVariables, variableMode: variableMode, DateTimeType: DateTimeType !== null && DateTimeType !== void 0 ? DateTimeType : FormulaDateType.NotDate }, rest))
        ]
    };
});
...
setSchemaTpl('optionsMenuTpl', function (config) {
    var manager = config.manager, rest = __rest(config, ["manager"]);
    // 设置 options 中变量集合
    function getOptionVars(that) {
        ...
        if (schema.options) {
            ...
            optionItem_1 = omit(optionItem_1, map(children, function (item) { return item === null || item === void 0 ? void 0 : item.label; }));
            var otherItem = map(keys(optionItem_1), function (item) { return ({
    ...
    return getSchemaTpl('textareaFormulaControl', __assign({ mode: 'normal', label: tipedLabel('选项模板', '自定义选项渲染模板，支持JSX、数据域变量使用'), name: 'menuTpl', variables: getOptionVars, requiredDataPropsVariables: true }, rest));
});
/**
 * 数据源绑定
 */
setSchemaTpl('sourceBindControl', function (schema) {
    if (schema === void 0) { schema = {}; }
    return (__assign({ type: 'ae-formulaControl', name: 'source', label: '数据', variableMode: 'tree', inputMode: 'input-group', placeholder: '请输入表达式', requiredDataPropsVariables: true }, schema));
});
...
setSchemaTpl('name', {
    label: tipedLabel('名字', '需要联动时才需要，其他组件可以通过这个名字跟当前组件联动'),
    name: 'name',
    type: 'input-text',
...
setSchemaTpl('reload', {
    ...
    label: tipedLabel('刷新目标组件', '可以指定操作完成后刷新目标组件，请填写目标组件的 <code>name</code> 属性，多个组件请用<code>,</code>隔开，如果目标组件为表单项，请先填写表单的名字，再用<code>.</code>连接表单项的名字如：<code>xxForm.xxControl</code>。另外如果刷新目标对象设置为 <code>window</code>，则会刷新整个页面。'),
...
setSchemaTpl('onlyClassNameTab', function (label) {
    ...
    return {
        ...
        body: getSchemaTpl('collapseGroup', [
            {
                title: 'CSS类名',
                body: [getSchemaTpl('className', { label: label })]
            }
        ])
    };
});
...
setSchemaTpl('combo-container', function (config) {
    ...
    if (isObject(config)) {
        ...
        return __assign(__assign(__assign({}, config), (itemsWrapperClassName ? { itemsWrapperClassName: itemsWrapperClassName } : {})), (itemClassName ? { itemClassName: itemClassName } : {}));
    }
    return config;
});
/**
...
setSchemaTpl('searchable', function (schema) {
    if (schema === void 0) { schema = {}; }
    return getSchemaTpl('switch', __assign({ label: '可检索', name: 'searchable' }, schema));
});
setSchemaTpl('sortable', function (schema) {
    if (schema === void 0) { schema = {}; }
    return getSchemaTpl('switch', __assign({ label: '可排序', name: 'sortable' }, schema));
});
...
setSchemaTpl('dateShortCutControl', function (schema) {
    if (schema === void 0) { schema = {}; }
    return __assign({ type: 'ae-DateShortCutControl', name: 'shortcuts' }, schema);
});
...
setSchemaTpl('virtualThreshold', {
    ...
    step: 1,
    precision: 0,
    label: tipedLabel('虚拟列表阈值', '当选项数量超过阈值后，会开启虚拟列表以优化性能'),
...
setSchemaTpl('primaryField', {
    type: 'input-text',
    name: 'primaryField',
    label: tipedLabel('主键', '每行记录的唯一标识符，通常用于行选择、批量操作等场景。'),
    pipeIn: function (value, formStore) {
        ...
        if (value == null || typeof value !== 'string') {
            return rowSelection &&
                (rowSelection === null || rowSelection === void 0 ? void 0 : rowSelection.keyField) &&
