...
import type { DSFeatureType, GenericSchema, CRUDScaffoldConfig, FormScaffoldConfig } from './type';
...
export interface DSBuilderInterface<T extends DSBuilderBaseOptions = DSBuilderBaseOptions> {
    /** 数据源中文名称，主要用于前端展示 */
    ...
    /** 当前上下文可以使用的字段 */
    getAvailableContextFields(options: Omit<T, 'renderer'>, target: EditorNodeType): Promise<any>;
    /** 获取CRUD列表字段 */
    getCRUDListFields?: <F extends Record<string, any>>(options: T) => Promise<F[]>;
    /** 获取CRUD简单查询字段 */
    getCRUDSimpleQueryFields?: <F extends Record<string, any>>(options: T) => Promise<F[]>;
    /** 构建简单查询表单项 */
    buildSimpleQueryCollectionSchema?: (options: T) => Promise<GenericSchema[] | undefined>;
    /** 获取CRUD高级查询字段 */
    getCRUDAdvancedQueryFields?: <F extends Record<string, any>>(options: T) => Promise<F[]>;
    /** 构建高级查询 */
    buildAdvancedQuerySchema?: (options: T) => Promise<GenericSchema | undefined>;
    /** 获取CRUD模糊查询字段 */
    getCRUDFuzzyQueryFields?: <F extends Record<string, any>>(options: T) => Promise<F[]>;
    /** 构建模糊查询 */
    ...
    /** 构建 CRUD 的顶部工具栏 */
    buildCRUDHeaderToolbar?: (options: T, componentId?: string) => Promise<GenericSchema>;
    /** 表格的表头查询 */
    buildCRUDFilterSchema(options: T, componentId?: string): Promise<any>;
    /** 表格单列 */
    buildCRUDColumn?: (field: Record<string, any>, options: T, componentId?: string) => Promise<any>;
    /** 表格操作列 */
...
export declare abstract class DSBuilder<T extends DSBuilderBaseOptions> implements DSBuilderInterface<T> {
    ...
    /** 获取功能场景的label */
    getFeatLabelByKey(feat: DSFeatureType): string;
    filterByFeat(feat: any): any;
    abstract match(schema?: any, key?: string): boolean;
    abstract getContextFields(options: T): Promise<any>;
    abstract getAvailableContextFields(options: Omit<T, 'renderer'>, target: EditorNodeType): Promise<any>;
    ...
    /** 基于 schema 还原CRUD脚手架配置 */
    abstract guessCRUDScaffoldConfig<T extends CRUDScaffoldConfig<any, any>>(options: {
        schema: GenericSchema;
        [propName: string]: any;
    }): Promise<T> | T;
    /** 基于 schema 还原Form脚手架配置 */
    abstract guessFormScaffoldConfig<T extends FormScaffoldConfig<any, any>>(options: {
        schema: GenericSchema;
        [propName: string]: any;
    }): Promise<T> | T;
...
export declare const builderFactory: Map<string, DSBuilderClass>;
/** 注册数据源构造器 */
export declare const registerDSBuilder: (klass: DSBuilderClass) => void;
