...
var CRUDCardsPlugin = /** @class */ (function (_super) {
    __extends(CRUDCardsPlugin, _super);
    function CRUDCardsPlugin(manager) {
        ...
        _this.description = '围绕卡片列表的数据增删改查. 负责数据的拉取，分页，单条操作，批量操作，排序，快速编辑等等功能，集成查询条件。';
        _this.order = -1000;
        _this.$schema = '/schemas/CRUD2CardsSchema.json';
        _this.docLink = '/amis/zh-CN/components/crud2';
        _this.previewSchema = _this.generatePreviewSchema('cards');
        _this.scaffold = _this.generateScaffold('cards');
        /** 非实体数据源走默认构建 */
    ...
    CRUDCardsPlugin.prototype.renderColumnsControl = function (context) {
    ...
    CRUDCardsPlugin.prototype.renderToolbarCollapse = function (context) {
        var builder = this.dsManager.getBuilderBySchema(context.node.schema);
        return {
            order: 20,
            title: '工具栏',
    ...
    CRUDCardsPlugin.prototype.renderFiltersCollapse = function (context) {
        var builder = this.dsManager.getBuilderBySchema(context.node.schema);
        var collection = [];
    ...
    var _a, _b, _c;
    CRUDCardsPlugin.id = 'CardsCRUDPlugin';
    __decorate([
        autobind,
