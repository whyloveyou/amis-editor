...
import { registerEditorPlugin, BasePlugin, diff, defaultValue, getSchemaTpl, CodeEditor as AmisCodeEditor, tipedLabel } from 'amis-editor-core';
                                    ...
                                    body: [
                                        ...
                                        */
                                        getSchemaTpl('apiControl', {
                                            label: tipedLabel('数据接口', '接口可以返回echart图表完整配置，或者图表数据，建议返回图表数据映射到 Echarts 配置中'),
                                            mode: 'normal'
                                        ...
                                        {
                                            name: 'interval',
                                            label: tipedLabel('定时刷新间隔', '设置后将自动定时刷新，最小3000, 单位 ms'),
                                            type: 'input-number',
                                            step: 500,
                                            // visibleOn: 'chartDataType === "dataApi" && data.api',
                                            visibleOn: 'data.api.url',
                                        ...
                                        getSchemaTpl('expressionFormulaControl', {
                                            evalMode: false,
                                            label: tipedLabel('跟踪表达式', '如果这个表达式的值有变化时会更新图表，当 config 中用了数据映射时有用'),
                                        ...
                                        }),
                                        {
                                            ...
                                            mode: 'normal',
                                            // type: 'json-editor',
                                            label: tipedLabel('Echarts 配置', '支持数据映射，可将接口返回的数据填充进来')
                                        },
                                        {
                                            name: 'dataFilter',
                                            ...
                                            mode: 'normal',
                                            label: tipedLabel('数据映射（dataFilter）', '如果后端没有直接返回 Echart 配置，可以自己写一段函数来包装'),
                                            renderLabel: true,
                                            params: [
                                                {
                                                    label: 'config',
                                        ...
                                        getSchemaTpl('switch', {
                                            label: tipedLabel('Chart 配置完全替换', '默认为追加模式，新的配置会跟旧的配置合并，如果勾选将直接完全覆盖'),
                        ...
                        body: getSchemaTpl('collapseGroup', __spreadArray([
                            {
                                ...
                                body: [
                                    getSchemaTpl('style:widthHeight', {
                                        ...
                                        heightSchema: {
                                            label: tipedLabel('高度', '默认高度为300px，值单位默认为 px，也支持百分比等单位 ，如：100%'),
